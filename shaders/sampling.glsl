...
float shlickMaskingTerm(float NdotL, float NdotV, float roughness)
{
...
vec3 shlickFresnel(vec3 f0, float LdotH)
{
    return f0 + (1.0 - f0) * pow(1.0 - LdotH, 5.0);
}
vec3 sampleGGX(vec3 N, vec3 dir, float roughness, float r1, float r2)
{
    float a2 = roughness * roughness;
    float phi = TWO_PI * r1;
