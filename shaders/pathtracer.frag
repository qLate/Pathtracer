...
bool IntersectTriangle(inout Ray ray, Triangle tri)
{
    ...
    ray.closestT = t;
    ray.materialIndex = int(tri.materialIndex);
    ray.surfaceNormal = getTriangleNormalAt(tri, u, v, false);
    ray.interPoint = hitPos;
    ray.uvPos = vec2(tri.vertices[0].posU.w, tri.vertices[0].normalV.w) + u * tri.texVec.xy + v * tri.texVec.zw;
    ...
    for (int x = 0; x < pointSize.x; ++x)
    {
        for (int y = 0; y < pointSize.y; ++y)
        {
            for (int z = 0; z < pointSize.z; ++z)
            {
                ...
                if (dist > areaLight.properties2.x) continue;

                vec3 dir = normalize(lightPoint - ray.interPoint);
                if (castShadowRays(Ray(lightPoint, -dir, dist, RAY_DEFAULT_ARGS_WO_DIST))) continue;
